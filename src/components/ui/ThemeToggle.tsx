...
export function ThemeToggle() {
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      {open && (
        <div className="absolute right-0 top-full mt-2 w-44 rounded-lg border border-[var(--border)] bg-[var(--bg-soft)] shadow-xl z-50 py-1 text-[var(--fg)]">
          {/* 主题切换 */}
          <div className="px-3 py-2">
            <div className="text-xs text-[var(--fg-muted)] mb-2">主题</div>
            <button
              onClick={() => toggleTheme()}
              className="flex w-full items-center gap-2 rounded px-2 py-1.5 text-sm text-[var(--fg)] border border-[var(--border)] hover:bg-[var(--border)] transition-colors"
            >
              {theme === "dark" ? (
                <svg
                  width="14"
                ...
                <button
                  ...
                  className={`flex-1 rounded px-1 py-1 text-xs transition-colors ${
                    fontSize === opt.value
                      ...
                      : "border border-[var(--border)] text-[var(--fg)] hover:bg-[var(--border)]"
                  }`}
                >
                  {opt.label}
                </button>
